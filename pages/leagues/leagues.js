...
Page({
  ...
  onLoad() {
    ...
    // ★ 2026-09-10（隐私指引审核整改）：去掉 wx.getSystemInfoSync 兜底 ——
    //   该旧接口官方说明「会获取系统权限，可能触发授权弹窗」。改用 wx.getWindowInfo，
    //   取不到则用 667 保守默认值（仅影响「上滚一屏」阈值判断，无功能损失）。
    try {
      const wi = wx.getWindowInfo ? wx.getWindowInfo() : null;
      this._winH = (wi && (wi.windowHeight || wi.screenHeight)) || 667;
