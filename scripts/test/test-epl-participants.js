...
check('Infobox league 的 team_number 应为 16', () => {
...
check('TeamCard FormerParticipants 过滤：应移除已替换队伍', () => {
...
check('TeamCard 无 qualifier 字段时 status 应为 TBD', () => {
  const wikitext = [
    '{{TeamCard',
    '|team=Team Gamma',
    '|p1=Player1',
...
check('TeamCard team= 字段含 [[...]] 链接时正确提取', () => {
  const wikitext = [
    '{{TeamCard',
    '|team=[[Team Spirit|TS]]',
