// supabase/functions/liquipedia-proxy/parse.ts
// ★★ 本文件是 `utils/liquipedia-parse.js` 的 **ESM 镜像**（微信云开发退役 · LP 解析链迁移）。
//   来源：utils/liquipedia-parse.js（与 cloudfunctions/aggregation/liquipedia-parse.js **逐字一致**，
//         同 md5，可作单一实现源）；本文件仅做两处机械转换：
//           · `module.exports = { … }` → `export { … }`（Deno/ESM 要求）
//           · 其余解析逻辑**逐字未改** —— 迁移的保真度依赖于此。
//   ⚠️ 改动请只改 `utils/liquipedia-parse.js`，然后重新生成本镜像；不要手改本文件。
//   验证方式：迁移后对同一 slug 对比「EF 返回」与「云函数返回」的 JSON（本会话即用此法）。
// ============================================================================
...
var TZ_OFFSET = {
  ...
  'ICT': 7,               // 印度支那
  'WIB': 7,               // 印尼西部
  // ★ v3 优化项34（2026-08-03）：补常见赛事时区。
  // ALMT 缺失曾导致 GotF 2026（哈萨克斯坦，阿斯塔纳）全部时间偏晚 6h：
  // 18:50 ALMT 被当 18:50 UTC → 本应 LIVE 的 20:50 北京对局被误判 upcoming。
  // 2026-08-03 修正：ALMT = UTC+5（哈萨克斯坦 2024-03 起全国统一 AQTT/UTC+5），
  // 勿用历史 ALMT=UTC+6。交叉验证：GotF 排期 18:50 ALMT=13:50 UTC vs OpenDota
  // 实际开赛 13:52:08 UTC（差 2 分钟=排期 vs 实际）；+6 则差 1 小时不吻合。
  'ALMT': 5,              // 哈萨克斯坦（阿斯塔纳）—— 2024-03 起 UTC+5
  'YEKT': 5,              // 叶卡捷琳堡
  'EEST': 3,              // 东欧夏令时
  'IDT': 3,               // 以色列夏令时
  'AST': 3,               // 阿拉伯标准时间（利雅得；注意与美东大西洋 -4 歧义，电竞赛事多为中东 +3）
  'SGT': 8, 'HKT': 8,     // 新加坡 / 香港
  'JST': 9,               // 日本
  'SAST': 2, 'CAT': 2,    // 南非 / 中非
  'WAT': 1,               // 西非
  'AEST': 10, 'AEDT': 11, // 澳大利亚
  'NZST': 12              // 新西兰
};
...
export { parseTemplate, splitTopLevel, stripWikitextMarkup, stripTags, parsePrizePool, collectDates, findTemplateEnd, extractLink, parseOpponentBlock, parseTeamCardBlock, parseParticipants, parseLeagueMetadata, parseScheduledMatches, parseGroupStandings, parseBrackets, parseBoFormat, parseTeamLogo, parseLeagueTier, extractTeamOpponent };
