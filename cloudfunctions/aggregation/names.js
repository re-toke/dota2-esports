// cloudfunctions/aggregation/names.js
// ★★ 本文件是 `utils/names.js` 的**镜像**（与 league-canon-map.js / curation-shared.js 同一模式）：
//   微信云函数是**独立 bundle**，无法 require 主包文件，故此处保留一份同实现的副本。
//   ⚠️ **改动请只改 `utils/names.js`（单一实现），然后同步本镜像**；
//      守卫 `scripts/test/test-sources.js` 会比对「**剥掉注释后**的代码」是否逐字一致，
//      不一致即 FAIL —— 所以注释可以不同（本段说明即为此而写），代码必须完全相同。
// ============================================================================
// utils/names.js
// ============================================================================
